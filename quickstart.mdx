...
<AccordionGroup>
    ...
    <Accordion icon="shield-check" title="Understand authentication">
        ...
        credentials for a JWT token 2. Include the token in the `Authorization` header for protected routes 3. Tokens
        expire after 1 hour for security
        <Tip>
            Always store your issued API secret securely and never expose it in client-side code. These credentials are
...
Depending on whether a campaign already exists and whether automatic creation applies, you may get **200** with `campaign_link`, or **201** with either `ad_creation_link` and `campaign_id`, or `campaign_link` only. See [Campaigns](/api-reference/endpoint/campaign).
...
<Accordion icon="book-open" title="Explore the full API">
    ...
    - **[Campaign Management](/api-reference/endpoint/campaign)** — Create or resolve campaigns for your clients
